...
ggplot(aes(x=position, y=fst)) +
  geom_point(size=0.002, alpha=0.5) +
  theme_cowplot()
## LG08
filter(fst, lg=="LG08") %>%
ggplot(aes(x=position, y=fst)) +
...
Each RAD tag is 150 bp. Note that `n_rad_tag` are numbers of RAD tags per chromosome. For Atlantic cod, the genome size is ~20x of the size of this simulated chromosome. 

```{r eval=T, warning=F, message=F}
maf_final_250 <- allele_count_to_fst("../two_pop_sim_fixed_m2_pos_lower_s_lower_r/rep_1/", n_rad_tag = 250) %>%
  mutate(n_rad_tag = 250)
maf_final_2500 <- allele_count_to_fst("../two_pop_sim_fixed_m2_pos_lower_s_lower_r/rep_1/", n_rad_tag = 2500) %>%
  mutate(n_rad_tag = 2500)
maf_final_25000 <- allele_count_to_fst("../two_pop_sim_fixed_m2_pos_lower_s_lower_r/rep_1/", n_rad_tag = 25000) %>%
  mutate(n_rad_tag = 25000)
maf_final <- bind_rows(maf_final_250, maf_final_2500, maf_final_25000)
```

Note: having 25000 150pb RAD tags is sampling ~1/8 of the chromosome.

## Plot per SNP Fst

```{r eval=T, warning=F, message=F, fig.height=4.5, fig.width=15}
