...
for (coverage in c(0.25,0.5,1,2,4,8)){
  for (sample_size in c(5,10,20,40)){
...
```{r eval=T, warning=F, message=F, fig.height=8, fig.width=10}
ggplot(sfs_final, aes(x=frequency, y=value)) +
...
linear_model_nInd_5 <- group_by(joined_frequency_final_nInd_3, coverage, sample_size) %>%
  summarise(r_squared=summary(lm(estimated_frequency~frequency))$r.squared)
joined_frequency_final_nInd_5 %>%
  ggplot(aes(x=frequency, y=estimated_frequency)) +
  geom_point(alpha=0.1, size=0.2) +
  geom_smooth(method="lm", color="blue", size=1, se = F) +
  geom_text(data = linear_model_nInd_3, aes(x = 0.9, y = 0.2, label=round(r_squared,3)), color = 'blue',  parse = TRUE) +
  facet_grid(coverage~sample_size) +
  theme_cowplot()
```
...
```{r eval=T, warning=F, message=F}
filter(joined_frequency_final_nInd_4, coverage==0.5, sample_size==20) %>%
  mutate(error=frequency-estimated_frequency) %>%
