...
error_plot_combined <- joined_frequency_final_combined %>%
  ...
  geom_violin(outlier.shape = NA, size=0.8, scale="width", draw_quantiles = 0.5) +
  ...
  facet_grid(coverage~sample_size) +
  scale_x_discrete(labels=seq(0.1, 0.9, 0.2))  +
  scale_fill_viridis_d(labels=c("LC-WGS with even coverage", "LC-WGS with uneven coverage", "Pool-seq with uneven coverage"), begin = 0, end = 0.75, direction = -1) +
  scale_color_viridis_d(labels=c("LC-WGS with even coverage", "LC-WGS with uneven coverage", "Pool-seq with uneven coverage"), begin = 0, end = 0.75, direction = -1) +
...
frequency_plot_combined <- joined_frequency_final_combined %>%
  ...
  geom_point(size=0.01, alpha=1, show.legend = F) +
  geom_text(data = joined_summary_table, x = 0.12, aes(label=r_squared, color = design, y = 0.97-as.numeric(as.factor(design))/8), fontface = "bold", size=4.8, show.legend = F) +
  #geom_text(data = joined_summary_table, x = 2.8, aes(label=root_mean_error_squared, color = design, y = 1-as.numeric(as.factor(design))/14), fontface = "bold", size=5, show.legend = F) +
  geom_text(data = joined_summary_table, x = 0.82, aes(label=n, color = design, y = 0.4-as.numeric(as.factor(design))/8), fontface = "bold", size=4.8, show.legend = T) +
  annotate("text", x = 0.12, y = 0.97, label="R^2", parse=T , size=4.8) +
  #annotate("text", x = 2.8, y=1.01, label=RMSE", size=5) +
  annotate("text", x = 0.82, y = 0.4, label="SNP count", size=4.8) +
